...
class Levelling(commands.Cog):
    ...
    async def connect_database(self):
        self.db = await aiosqlite.connect('db/rank.db')
    ...
    async def on_message(self, message: discord.Message):
        ...
        if self.calculate_level(xp) > level:
            level += 1
            # 1,000
            await message.channel.send(f"Congratulations, {message.author.mention} You levelled up to {level:,}. `Type .rank to check xp`")
    ...
    async def make_rank_image(self, member: discord.Member, rank, level, xp, final_xp):
        ...
        imgrng = random.randint(1,8)        
        img = Image.open(f"images/banner{imgrng}.png")
        #img = Image.open(f"images/banner10.png")
        img = img.convert("RGB")
        #img = Image.new('RGB', (1000, 240), color = (25, 25, 25))
        ...
        # Working with fonts
        big_font = ImageFont.FreeTypeFont('fonts/LEMONMILK-Regular.otf', 60)
        medium_font = ImageFont.FreeTypeFont('fonts/LEMONMILK-Regular.otf', 40)
        small_font = ImageFont.FreeTypeFont('fonts/LEMONMILK-Regular.otf', 30)

        # Placing Level text (right-upper part)
        text_size = draw.textsize(f"{level}", font=big_font)
        offset_x = 1000-15 - text_size[0]
        offset_y = 5 
        draw.text((offset_x, offset_y), f"{level}", font=big_font, fill="#FFD700")
        text_size = draw.textsize('LEVEL', font=small_font)

        offset_x -= 5 + text_size[0]
        offset_y = 35
        draw.text((offset_x, offset_y), "LEVEL", font=small_font, fill="#FFD700")
        ...
        # Drawing Rectangle
        draw.rectangle((bar_offset_x, bar_offset_y, pbar_offset_x_1, bar_offset_y_1), fill="#FFD700")
        # Left circle
        draw.ellipse((bar_offset_x - circle_size//2, bar_offset_y, bar_offset_x + circle_size//2, bar_offset_y + circle_size), fill="#FFD700")
        # Right Circle
        draw.ellipse((pbar_offset_x_1 - circle_size//2, bar_offset_y, pbar_offset_x_1 + circle_size//2, bar_offset_y_1), fill="#FFD700") #gold:#FFD700
